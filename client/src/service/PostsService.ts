...
import { CreatePostResponse } from "../models/response/CreatePostResponse";
...
export default class PostService {
    ...
    static async CreatePost(title: string, description: string, topics: number[]): Promise<AxiosResponse<CreatePostResponse>> {
        return $api.post<CreatePostResponse>('/posts', { title, description, topics })
    }

    static async CreatePostImage(id: string, imageUri: any): Promise<AxiosResponse<CreatePostResponse>> {
        return $api.post<CreatePostResponse>(`/posts/${id}/image`, imageUri)
    }
